...
//底部地图二维码
	...
	function effect() {
		//头部消息下拉
		$(".t-link>.newsLi").hover(function() {
			$(this).children("#letterMenu").stop().slideDown(100);
		}, function() {
			$(this).children("#letterMenu").stop().slideUp(100);
		})
		//头部用户登录下拉
		$(".t-link>.userNameLi").click(function() {
			$(this).children("#userMenu").slideToggle("linear");
		})
		//课程列表说明层 
	...
	function dialog(dTitle,conent,idnex,url,loginType) {
		var cssText = "pt20 pb20 pl20 pr20 ";
		if (idnex == 8) {
			cssText = "";
		}
		var oBg = $('<div class="bg-shadow"></div>').appendTo($("body")),
			dialogEle = $('<div class="dialog-shadow"><div class="dialog-ele"><h4 class="d-s-head pr"><a href="javascript:void(0)" title="关闭" class="dClose icon16 pa">&nbsp;</a><span class="d-s-head-txt">'+dTitle+'</span></h4><div id="dcWrap" class="' + cssText + 'of" style=""></div></div></div>').appendTo($("body")).addClass("animate-enter animated flyIn5");				
			$.ajax({
