...
function ajaxPageNew(url,parameters,pageNum,callBack,addMoreType){
	if (addMoreType == 0) {
		$(".kcArea").html('');
		$(".jyArea").html('');
		$(".wzArea").html('');
...
function goPageAjaxNew(pageNum,type){
	if (type == 1) {
		$('#addMoreButtonKC').remove();
	} else if (type == 2) {
		$('#addMoreButtonJY').remove();
	} else if (type == 3) {
		$('#addMoreButtonWZ').remove();
	} else if (type == 4) {
		$('#addMoreButtonGW').remove();
	}
	ajaxPageNew(ajaxUrl,ajaxparameters,pageNum,ajaxcallBack,1);
}
//判断是否登陆并进行一些操作
function checkLogin(){
	//登陆才可观看
...
function addImage() {   
	$("#img").attr("src","/randomCode?v="+Math.random());
}
