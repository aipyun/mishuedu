...
namespace VipPlugin\Biz\Importer;
...
class VipImporter extends Importer
{
    ...
    public function import(Request $request)
    {
        ...
        foreach ($userDataArray as $userData) {
            ...
            $this->getVipService()->becomeMember($user['id'], $viplevel['id'], $userData['viplevelTime'], 'month', 0);
    ...
    public function check(Request $request)
    {
        ...
        for ($row = 3; $row <= $highestRow; ++$row) {
            ...
            if ($userData['viplevelTime'] == 0 || $userData['viplevelTime'] > 999) {
                $checkInfo[] = '第'.$row.'行的用户会员时效必须在0-999之间，已跳过';
                continue;
            }
    ...
    {
        return $this->render('VipPlugin:VipAdmin:import.html.twig', array(
    ...
    protected function checkNecessaryFields($data = array())
    {
        $data = implode('', $data);
        $data = $this->trim($data);
        ...
        $viplevelName_array = explode('会员等级', $data);
    ...
    protected function getFieldSort($excelField = array(), $fieldArray = array())
    {
        $fieldSort = array();
    ...
    protected function getUserData($userData = null)
    {
        $user = null;
    ...
    protected function checkPassedRepeatData($passedUsers = array())
    {
        $ids = array();
        $rows = array();
